...
import {
  StatusBar, MagneticButton, TiltCard, Confetti,
  useCountUp, usePrefersReducedMotion,
} from './ui'

const Scene = lazy(() => import('./Scene'))

/* Isolate WebGL: if 3D fails on a device, the app keeps working. */
...
Return ONLY raw JSON. No markdown. No code fences. No preamble. No trailing text.
...
"type" must be exactly one of: "rare", "gotcha", "scenario", "bestpractice"
...
const RANKS = [
  { name:'Rookie', min:0, icon:'ti-seedling' },
  { name:'Apprentice', min:120, icon:'ti-bolt' },
  { name:'Practitioner', min:320, icon:'ti-tools' },
  { name:'Specialist', min:640, icon:'ti-star' },
  { name:'Expert', min:1080, icon:'ti-flame' },
  { name:'Architect', min:1680, icon:'ti-building-arch' },
...
const screenV = {
  ...
  animate: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.5, ease: [.2,.8,.2,1] } },
  exit:    { opacity: 0, y: -14, scale: 0.995, transition: { duration: 0.3, ease: [.4,0,1,1] } },
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setBooting(false), reduced ? 200 : 1700)
  ...
  const spawnCoin = (x, y, val) => {
    const id = ++coinId.current
    setCoins((c) => [...c, { id, x, y, val }])
  }
  ...
  const launch = useCallback(async (lv) => {
    ...
    try {
      ...
      catch { throw new Error('Could not parse the generated questions. Please try again.') }
      if (!Array.isArray(parsed.questions) || parsed.questions.length < 5)
        throw new Error('The generator returned too few questions. Please try again.')
  ...
  const advance = () => {
    ...
    if (idx + 1 >= qs.length) {
      ...
      Sound.finish(pctv >= 70)
      if (changed) setTimeout(() => Sound.rankUp(), 650)
      setPhase('results')
    } else {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {coins.map((c) => (
          <motion.div key={c.id} className="coin"
            initial={{ opacity: 0, scale: 0.4, x: c.x, y: c.y }}
            animate={{ opacity: 1, scale: 1, x: c.x, y: c.y - 78 }}
            exit={{ opacity: 0 }} transition={{ duration: 0.9, ease: 'easeOut' }}
            onAnimationComplete={() => removeCoin(c.id)}>
      ...
      <AnimatePresence>
        {booting && (
          <motion.div className="loader" key="loader" initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.6 }}>
            <div className="loader__inner">
              <div className="loader__logo"><CloudLogo size={92} /></div>
              <div className="loader__bar"><motion.div className="loader__fill" initial={{ width: '0%' }} animate={{ width: '100%' }} transition={{ duration: 1.4, ease: 'easeInOut' }} /></div>
      ...
      <div className="app-shell">
        ...
        <AnimatePresence mode="wait">
          {/* ───── HOME ───── */}
          {phase === 'home' && (
            <motion.div key="home" variants={screenV} initial="initial" animate="animate" exit="exit">
              <motion.header className="hero" variants={stagger} initial="initial" animate="animate">
                <motion.div className="hero__logo" variants={itemV}><CloudLogo size={104} /></motion.div>
                <motion.p className="hero__eyebrow" variants={itemV}>Salesforce Career Ladder</motion.p>
                <motion.h1 className="hero__title" variants={itemV}>Master the Cloud</motion.h1>
                <motion.p className="hero__sub" variants={itemV}>
                ...
                <motion.div className="hero__stats" variants={itemV}>
              ...
              {/* rank HUD */}
              <motion.div className="rankhud" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }}>
                ...
                <div className="rankhud__body">
                  ...
                  <div className="rankhud__bar"><motion.div className="rankhud__fill" initial={{ width: 0 }} animate={{ width: `${rk.prog}%` }} transition={{ delay: 0.35, duration: 0.8, ease: [.2,.8,.2,1] }} /></div>
                    ...
                    <TiltCard style={lvVars(lv)} onClick={() => launch(lv)}>
                      <div className="lvcard" style={lvVars(lv)} onMouseEnter={() => Sound.hover()}>
                        <div className="lvcard__top">
                          ...
                          {best[lv.id] !== undefined && <span className="lvcard__best">best {best[lv.id]}/10</span>}
                        </div>
            ...
            <motion.div key="quiz" variants={screenV} initial="initial" animate="animate" exit="exit" style={lvVars(level)}>
              <div className="qhead">
                ...
                <div className="qhead__right">
                  <AnimatePresence>
                    {streak >= 2 && (
                      <motion.span className="combo" key="combo" initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }}>
                        <i className="ti ti-flame" /> {streak}× streak
                      </motion.span>
                    )}
                  </AnimatePresence>
              ...
              {(() => {
                const q = qs[idx], tm = TYPE_CFG[q.type] || TYPE_CFG.scenario
                return (
              ...
              <motion.div className="opts" key={`opts${idx}`} variants={stagger} initial="initial" animate="animate">
                {qs[idx].options.map((opt, i) => {
                  ...
                  return (
                    <motion.button key={i} className={c} variants={itemV} onClick={(e) => pick(i, e)} disabled={revealed}
                      whileHover={!revealed ? { x: 4 } : {}} whileTap={!revealed ? { scale: 0.985 } : {}}>
              ...
              {revealed && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                  ...
                  <AnimatePresence>
                    {showExp && (
                      <motion.div className="exp" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} style={{ overflow: 'hidden' }}>
                        <div className="exp__head"><i className="ti ti-bulb" /><span>Explanation</span></div>
                        <p className="exp__body">{qs[idx].explanation}</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                  <MagneticButton className="btn--lv" onClick={advance}>
          ...
          {phase === 'results' && level && (
            <motion.div key="results" variants={screenV} initial="initial" animate="animate" exit="exit" style={lvVars(level)}>
              <ResultsScreen
                ...
                xpGain={xpGain} rankUp={rankUp} rankName={rk.r.name}
                onReview={(i) => { Sound.click(); setRevIdx(i); setPhase('review') }}
                onRetry={() => launch(level)} onHome={() => { Sound.click(); setPhase('home') }}
              />
            </motion.div>
          )}
            ...
            return (
              <motion.div key="review" variants={screenV} initial="initial" animate="animate" exit="exit" style={lvVars(level)}>
                <div className="rev__nav">
                  <MagneticButton className="btn--ghost" style={{ width: 'auto', padding: '9px 16px' }} onClick={() => { Sound.click(); setPhase('results') }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <MagneticButton className="btn--ghost" style={{ flex: 1 }} disabled={revIdx === 0} onClick={() => { Sound.click(); setRevIdx((p) => Math.max(0, p - 1)) }}>
                    <i className="ti ti-arrow-left" /> Prev
                  </MagneticButton>
                  <MagneticButton className="btn--lv" style={{ flex: 1 }} disabled={revIdx === answers.length - 1} onClick={() => { Sound.click(); setRevIdx((p) => Math.min(answers.length - 1, p + 1)) }}>
...
/* ───────────── RESULTS SCREEN ───────────── */
function ResultsScreen({ level, answers, score, best, xpGain, rankUp, rankName, onReview, onRetry, onHome }) {
  const reduced = usePrefersReducedMotion()
    ...
    <>
      ...
      <div className="res">
        <div className="res__ring">
          <svg viewBox="0 0 168 168">
            ...
            <motion.circle cx="84" cy="84" r={R} fill="none" stroke="var(--lvc)" strokeWidth="9" strokeLinecap="round"
              strokeDasharray={C} initial={{ strokeDashoffset: reduced ? offset : C }} animate={{ strokeDashoffset: offset }}
              transition={{ duration: reduced ? 0 : 1.1, ease: [.2,.8,.2,1] }} style={{ filter: 'drop-shadow(0 0 8px var(--lvg))' }} />
          </svg>
        ...
        {xpGain > 0 && (
          <div className="res__xp">
            <i className="ti ti-coin" /> +{xpGain} XP earned
            {rankUp && <span className="res__rankup"><i className="ti ti-arrow-up" /> {rankName}!</span>}
          </div>
        )}
      ...
      <div className="breakdown">
        ...
        <div className="breakdown__grid">
          {answers.map((a, i) => (
            <button key={i} className={`qchip ${a.sel === a.correct ? 'yes' : 'no'}`} onClick={() => onReview(i)}>{i + 1}</button>
          ))}
        </div>
      </div>
      ...
      <div className="stack">
        ...
        <MagneticButton className="btn--primary" onClick={onRetry}><i className="ti ti-refresh" /> New set — {level.label}</MagneticButton>
        <MagneticButton className="btn--ghost" onClick={onHome}><i className="ti ti-arrow-left" /> Choose a different level</MagneticButton>
      </div>
    </>
  )
}
